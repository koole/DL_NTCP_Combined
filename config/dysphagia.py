...
data_dir = os.path.join(save_root_dir, 'suzanne_dysphagia')  # Change this one to the data directory!
exp_root_dir = os.path.join(root_path, 'experiments', 'dysphagia_test')   # Change this one so that each experiment is put in its own folder!
...
clinical_variables_linear_units = [64, 8]  # (Only if len(features_dl) > 0.) None | list of ints
clinical_variables_dropout_p = [0.1303721508792322, 0.49162348783424814]  # Should have the same length as `clinical_variables_linear_units`
use_bias = True
