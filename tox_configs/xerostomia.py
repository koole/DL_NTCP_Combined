...
filename_stratified_sampling_full_csv = (
    "stratified_features_full_DAN.csv"  # output file
)
...
data_aug_p = (
    0.5  # Probability of a data augmentation transform: data_aug_p=0 is no data aug.
)
data_aug_strength = (
    1  # Strength of data augmentation: strength=0 is no data aug (except flipping).
)
...
model_name = (
    "dcnn_lrelu"  # ['cnn_lrelu', 'convnext_tiny', 'convnext_small', 'convnext_base',
)
...
strides = [
    ...
    [2] * 3,
    [2] * 3,
]  # strides>2 currently not supported.
...
clinical_variables_linear_units = (
    None  # (Only if len(features_dl) > 0.) None | list of ints
)
...
step_size_up = (
    79 * 15
)  # (CyclicLR) Number of training iterations in the increasing half of a cycle.
