...
data_dir = os.path.join(
    save_root_dir, "suzanne_dysphagia"
)  # Change this one to the data directory!
exp_root_dir = os.path.join(
    root_path, "experiments", "dysphagia_test"
)  # Change this one so that each experiment is put in its own folder!
...
strata_groups = [
    ...
    "CT_Artefact",
    "Photons",
]  # , 'Year_treatment_2cat']  # (Stratified Sampling). Note: order does not matter.
...
cache_dir = os.path.join(
    root_path, "persistent_cache"
)  # (only if dataset_type='persistent')
...
rand_cropping_size = [
    ...
    96,
    96,
]  # (REDUNDANT)  # OLD [100, 100, 100]  # (only for training data)
input_size = [
    96,
    96,
    96,
]  # OLD [100, 100, 100]  # if rand_cropping_size==input_size, then no resizing will be applied

resize_mode = "area"  # (only if perform_resize=True). Algorithm used for upsampling.
# CT
ct_a_min = (
    -200
)  # OLD data_preproc_config.ct_min if data_preproc_config.perform_clipping else None
...
seg_orig_labels = [
    x for x in range(len(data_preproc_config.structures_uncompound_list) + 1)
]  # No need to change
...
model_name = (
    "resnet_lrelu"  # ['cnn_lrelu', 'convnext_tiny', 'convnext_small', 'convnext_base',
)
...
pooling_conv_filters = (
    None  # Either int or None (i.e. no pooling conv before flattening).
)
...
linear_units = [16]
dropout_p = [0.0012559377378828299]  # Should have the same length as `linear_units`
...
clinical_variables_linear_units = [
    64,
    8,
]  # (Only if len(features_dl) > 0.) None | list of ints
clinical_variables_dropout_p = [
    0.1303721508792322,
    0.49162348783424814,
]  # Should have the same length as `clinical_variables_linear_units`
use_bias = True
...
kaiming_a = math.sqrt(
    5
)  # [math.sqrt(5) (default), lrelu_alpha]. Only used when kaiming_nonlinearity = 'leaky_relu'.
kaiming_mode = "fan_in"  # ['fan_in' (default), 'fan_out'].
kaiming_nonlinearity = (
    "leaky_relu"  # ['leaky_relu' (default), 'relu', 'selu', 'linear']. When using
)
...
optimizer_name = "madgrad"  # ['acc_sgd', 'ada_belief', 'ada_bound', 'ada_bound_w', 'ada_hessian', 'ada_mod', 'adam',
# 'adam_w', 'apollo', 'diff_grad', 'madgrad', 'novo_grad', 'qh_adam', 'qhm', 'r_adam', 'ranger_21', 'ranger_qh',
# 'rmsprop', 'pid', 'sgd', 'sgd_w', 'swats', 'yogi']
optimizer_name_next = (
    []
)  # [] | ['sgd']. Next optimizers after nr_epochs_not_improved_opt >= patience_lr.
...
label_weights = [
    1,
    1,
]  # [1, 1] (ce) | [1] (bce) | w_jj = (1 - /beta) / (1 - /beta^{n_samples_j}) (\beta = 0.9, 0.99) |
# wj=n_samples / (n_classes * n_samplesj). Rescaling (relative) weight given to each class, has to be list of size C.
loss_reduction = "mean"
label_smoothing = 0.00046837642148514773  # (LabelSmoothing) If 0, then no label smoothing will be applied. Currently only supported for
...
T_mult = (
    1  # (CosineAnnealingWarmRestarts) A factor increases after a restart. Default: 1.
)
...
step_size_up = (
    256  # (CyclicLR) Number of training iterations in the increasing half of a cycle.
)
...
manual_lr = [
    ...
    1e-5,
    1e-6,
]  # (Manual_LR) LR per epoch, if epoch > len(manual_lr), then use lr = manual_lr[-1].
