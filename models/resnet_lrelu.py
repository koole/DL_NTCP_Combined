...
class ResNet_LReLU(nn.Module):
    ...
    def __init__(
        ...
        self.layer1 = self._make_layer(
            block, block_inplanes[0], layers[0], shortcut_type, lrelu_alpha
        )
        self.layer2 = self._make_layer(
            block, block_inplanes[1], layers[1], shortcut_type, lrelu_alpha, stride=2
        )
        self.layer3 = self._make_layer(
            block, block_inplanes[2], layers[2], shortcut_type, lrelu_alpha, stride=2
        )
        self.layer4 = self._make_layer(
            block, block_inplanes[3], layers[3], shortcut_type, lrelu_alpha, stride=2
        )
        ...
        if (linear_units is None) or (len(linear_units) == 0):
            self.out_layer = Output(
                in_features=output_channel + clinical_variables_linear_units[-1],
                ...
                if self.clinical_variables_position + 1 == i:
                    self.linear_layers.add_module(
                        ...
                        torch.nn.Linear(
                            ...
                            + clinical_variables_linear_units[-1],
        ...
        if (self.n_features > 0) and (len(self.linear_layers) == 0):
            ...
            for i, layer in enumerate(self.linear_layers):
                ...
                if (
                    (self.n_features > 0)
                    and (i == 0)
                    and (self.clinical_variables_position == -1)
                ):
                    # Add features to flattened layer
                    x = torch.cat([x, features], dim=1)
