...
from data_preproc.data_preproc_functions import (
    copy_file,
    copy_folder,
...
def initialize(torch_version, device, create_folders=True):
    ...
    if create_folders:
        ...
        src_files = [
            ...
            if (
                x.endswith(".py")
                or x == data_preproc_config.filename_exclude_patients_csv
            )
        ]
    ...
    logger.my_print(
        "Torch.backends.cudnn.benchmark: {}.".format(torch.backends.cudnn.benchmark)
    )
...
def validate(model, dataloader, mean, std, mode, logger, save_outputs=True):
    ...
    with torch.no_grad():
        ...
        for data in dataloader:
            ...
            inputs = process_data.preprocess_inputs(
                inputs=inputs,
                ct_mean=mean[0],
                ct_std=std[0],
                rtdose_mean=mean[1],
            ...
            try:
                ...
                # BCE
                loss = loss_function(
                    outputs, torch.reshape(labels, outputs.shape).to(outputs.dtype)
                )
        ...
        auc_value = misc.compute_auc(
            y_pred_list=y_pred_list, y_true_list=y_list, auc_metric=auc_metric
        )
        mse_value = misc.compute_mse(
            y_pred_list=y_pred_list, y_true_list=y_list, mse_metric=mse_metric
        )
...
def train(
    ...
    logger.my_print(
        "Number of training iterations per epoch: {}.".format(train_num_iterations)
    )
    epoch = 0
    best_epoch = 0
    for epoch in range(max_epochs):
        ...
        for i, batch_data in tqdm(enumerate(train_dataloader)):
            ...
            if perform_augmix:
                for b in range(len(train_inputs)):
                    ...
                    train_inputs[b] = process_data.aug_mix(
                        arr=train_inputs[b],
            ...
            # Preprocess inputs and features
            # NOTE: these operations should also be performed in lr_finder.py
            train_inputs = process_data.preprocess_inputs(
                inputs=train_inputs,
                ct_mean=mean[0],
                ct_std=std[0],
                rtdose_mean=mean[1],
            ...
            try:
                ...
                train_loss = loss_function(
                    train_outputs,
                    torch.reshape(train_labels, train_outputs.shape).to(
                        train_outputs.dtype
                    ),
                )
            ...
            if grad_max_norm is not None:
                torch.nn.utils.clip_grad_norm_(
                    parameters=model.parameters(), max_norm=grad_max_norm
                )
        ...
        train_auc_value = misc.compute_auc(
            y_pred_list=train_y_pred_list,
            y_true_list=train_y_list,
            auc_metric=auc_metric,
        )
        train_mse_value = misc.compute_mse(
            y_pred_list=train_y_pred_list,
            y_true_list=train_y_list,
        ...
        if (epoch + 1) % config.eval_interval == 0:
            ...
            val_loss_value, val_mse_value, val_auc_value = validate(
                model=model,
            ...
            test_loss_value, test_mse_value, test_auc_value = validate(
                model=model,
            ...
            if val_auc_value > best_val_auc_value:
                ...
                torch.save(
                    model.state_dict(),
                    os.path.join(config.exp_dir, config.filename_best_model_pth),
                )
            ...
            logger.my_print(
                f"Best internal validation AUC: {best_val_auc_value:.3f} at epoch: {best_epoch}."
            )
            logger.my_print(
                f"Corresponding internal validation loss: {best_val_loss_value:.3f} at epoch: {best_epoch}."
            )
            logger.my_print(
                f"Corresponding internal validation MSE: {best_val_mse_value:.3f} at epoch: {best_epoch}."
            )
        ...
        if (epoch + 1 == 1) or ((epoch + 1) % config.plot_interval == 0):
            ...
            for patient_idx in range(
                min(config.max_nr_images_per_interval, len(plot_inputs))
            ):
                arr_list = [
                    plot_inputs[patient_idx][0].cpu() * std[0] + mean[0],
                    plot_inputs[patient_idx][1].cpu() * std[1] + mean[1],
                    plot_inputs[patient_idx][2].cpu(),
                ]
                cmap_list = [
                    data_preproc_config.ct_cmap,
                    data_preproc_config.rtdose_cmap,
                ...
                ticks_steps_list = [
                    vmax_list[0] - vmin_list[0],
                    vmax_list[1] - vmin_list[1],
                    1,
                ]
                segmentation_map_list = [arr_list[2] >= 0.5, None, None]
                colorbar_title_list = [
                    data_preproc_config.ct_colorbar_title,
                ...
                plot_multiple_arrs(
                    ...
                    cmap_list=cmap_list,
                    cmap_overlay_list=cmap_overlay_list,
                    colorbar_title_list=colorbar_title_list,
                    filename=os.path.join(
                        config.exp_figures_dir,
                        "epoch_{}_idx_{}.png".format(epoch + 1, patient_idx),
                    ),
                    vmin_list=vmin_list,
                    vmin_overlay_list=vmin_overlay_list,
                    vmax_list=vmax_list,
                    vmax_overlay_list=vmax_overlay_list,
        ...
        if nr_epochs_not_improved >= patience:
            logger.my_print(
                "No internal validation improvement during the last {} consecutive epochs. "
            ...
            return (
                ...
                train_mse_values_list,
                val_mse_values_list,
                test_mse_values_list,
                train_auc_values_list,
                val_auc_values_list,
        ...
        if nr_epochs_not_improved_lr >= patience_lr:
            ...
            if scheduler_name == "lr_finder":
                ...
                new_lr, optimizer = misc.learning_rate_finder(
                    model=model,
                    train_dataloader=train_dl,
                    val_dataloader=val_dl,
                    optimizer=optimizer,
                    loss_function=loss_function,
                    data_aug_p=data_aug_p,
                    data_aug_strength=data_aug_strength,
                    perform_augmix=perform_augmix,
                    mixture_width=mixture_width,
                    mixture_depth=mixture_depth,
                    augmix_strength=augmix_strength,
                    mean=norm_mean_dict[fold],
                    std=norm_std_dict[fold],
                ...
                if new_lr is None:
                    ...
                    for param_group in optimizer.param_groups:
                        param_group["lr"] = max(
                            last_lr * config.factor_lr, config.min_lr
                        )
            ...
            # ReduceLROnPlateau scheduler
                ...
                for param_group in optimizer.param_groups:
                    param_group["lr"] = max(
                        param_group["lr"] * config.factor_lr, config.min_lr
                    )
        ...
        if nr_epochs_not_improved_opt >= patience_opt:
            ...
            if len(optimizer_name_next) >= 1:
                ...
                for param_group in optimizer.param_groups:
                    lr = param_group["lr"]
                optimizer = misc.get_optimizer(
                    optimizer_name=optimizer_name_i,
                    model=model,
                    lr=lr,
                    momentum=momentum,
...
if __name__ == "__main__":
    ...
    for fold in range(cv_folds):
        ...
        if pretrained_path is not None:
            pretrained_path_i = os.path.join(
                pretrained_path, str(fold), config.filename_best_model_pth
            )
        else:
        ...
        if cv_folds > 1:
            if fold == 0:
                ...
                if cv_type == "stratified":
                    logger.my_print(
                        "Performing stratified {}-fold CV.".format(cv_folds)
                    )
        ...
        (
            ...
            test_dict=test_dict,
            features=features_dl,
            perform_data_aug=perform_data_aug,
            data_aug_p=data_aug_p,
            data_aug_strength=data_aug_strength,
            rand_cropping_size=rand_cropping_size,
            input_size=input_size,
            resize_mode=resize_mode,
            seg_orig_labels=seg_orig_labels,
        ...
        logger.my_print(
            "Input shape: {}.".format([batch_size, channels, depth, height, width])
        )
        ...
        model = misc.get_model(
            ...
            use_bias=use_bias,
            pretrained_path=pretrained_path_i,
            logger=logger,
        )
        ...
        if pretrained_path_i is not None:
            ...
            (pretrained_path_i is None)
            and (weight_init_name is not None)
            and ("efficientnet" not in model_name)
            and ("convnext" not in model_name)
            and ("resnext" not in model_name)
        ):
            # Source: https://pytorch.org/docs/stable/generated/torch.nn.Module.html
            model.apply(
                lambda m: misc.weights_init(
                    m=m,
            ...
            logger.my_print(
                "Using our own weights init scheme for {}.".format(model_name)
            )
        else:
            logger.my_print(
                "Using default PyTorch weights init scheme for {}.".format(model_name)
            )
        ...
        total_params = misc.get_model_summary(
            model=model,
            input_size=[
        ...
        if lr_finder_num_iter > 0 and (config.max_epochs > 0):
            lr, optimizer = misc.learning_rate_finder(
                ...
                std=norm_std_dict[fold],
                grad_max_norm=grad_max_norm,
                lr_finder_num_iter=lr_finder_num_iter,
                save_filename=config.filename_lr_finder_png,
                device=device,
                logger=logger,
            )
        starting_lr = lr
        ...
        (
            ...
            std=norm_std_dict[fold],
            optimizer=optimizer,
            optimizer_name_next=optimizer_name_next,
            dl_class=dl_class,
            train_dl_args_dict=train_dl_args_dict,
            logger=logger,
        )
        ...
        if max_epochs > 0:
            ...
            legend_list = [["Training", "Internal validation"]] * (len(y_list) - 2) + [
                None,
                None,
            ]
            misc.plot_values(
                ...
                save_filename=os.path.join(exp_dir, config.filename_results_png),
            )

            # Load best model
            model.load_state_dict(
                torch.load(os.path.join(exp_dir, config.filename_best_model_pth))
            )
            ...
            all_modes = ["train"] * len(train_patient_ids) + ["val"] * len(
                val_patient_ids
            )
            ...
            if test_dl is not None:
                ...
                if (test_y_pred_sum is None) and (test_y_sum is None):
                    ...
                    test_y_pred_sum = [
                        x + y for x, y in zip(test_y_pred_sum, test_y_pred_list)
                    ]
            ...
            misc.save_predictions(
                ...
                y_pred_list=all_y_pred_list,
                y_true_list=all_y_list,
                mode_list=all_modes,
                num_classes=num_classes,
        ...
        features_csv = os.path.join(
            config.data_dir, data_preproc_config.filename_features_csv
        )
        patient_ids_json = os.path.join(
            exp_dir, config.filename_train_val_test_patient_ids_json.format(fold)
        )
        (
            train_patient_ids_lr,
            train_y_pred_lr,
            train_y_lr,
            val_patient_ids_lr,
            val_y_pred_lr,
            ...
            submodels_features=data_preproc_config.submodels_features,
            features=features_lr,
            lr_coefficients=data_preproc_config.lr_coefficients,
            endpoint=endpoint,
            patient_id_length=patient_id_length,
            patient_ids_json=patient_ids_json,
            seed=seed,
            nr_of_decimals=nr_of_decimals,
            logger=logger,
        )
        ...
        train_auc_lr = misc.compute_auc(
            y_pred_list=train_y_pred_lr_list,
            y_true_list=train_y_lr_list,
            auc_metric=auc_metric,
        )
        ...
        val_auc_lr = misc.compute_auc(
            y_pred_list=val_y_pred_lr_list,
            y_true_list=val_y_lr_list,
            auc_metric=auc_metric,
        )
        ...
        all_modes_lr = ["train"] * len(train_patient_ids_lr) + ["val"] * len(
            val_patient_ids_lr
        )
        ...
        if test_dl is not None:
            ...
            test_auc_lr = misc.compute_auc(
                y_pred_list=test_y_pred_lr_list,
                y_true_list=test_y_lr_list,
                auc_metric=auc_metric,
            )
            ...
            # Save outputs to csv
            misc.save_predictions(
                patient_ids=test_patient_ids_lr,
                y_pred_list=test_y_pred_lr_list,
                y_true_list=test_y_lr_list,
                mode_list=mode_lr_list,
                num_classes=num_classes,
                model_name="lr",
                exp_dir=exp_dir,
                logger=logger,
            ...
            if (test_y_pred_lr_sum is None) and (test_y_lr_sum is None):
                ...
                test_y_pred_lr_sum = [
                    x + y for x, y in zip(test_y_pred_lr_sum, test_y_pred_lr_list)
                ]
            ...
            if (fold == cv_folds - 1) and (cv_folds > 1):
                ...
                if max_epochs > 0:
                    ...
                    misc.save_predictions(
                        ...
                        y_pred_list=test_y_pred_ens,
                        y_true_list=test_y_ens,
                        mode_list=mode_lr_list,
                        num_classes=num_classes,
                        model_name=model_name + "_ens",
                        exp_dir=exp_dir,
                        logger=logger,
                    )
                ...
                misc.save_predictions(
                    patient_ids=test_patient_ids_lr,
                    y_pred_list=test_y_pred_lr_ens,
        ...
        # Save predictions on training, validation and test set
        misc.save_predictions(
            patient_ids=all_patient_ids_lr,
            y_pred_list=all_y_pred_lr_list,
            y_true_list=all_y_lr_list,
            mode_list=all_modes_lr,
            num_classes=num_classes,
            model_name="lr_all",
            exp_dir=exp_dir,
            logger=logger,
        ...
        if max_epochs > 0:
            ...
            misc.create_results(
                ...
                train_auc=train_auc_value,
                train_auc_lr=train_auc_lr,
                val_auc=val_auc_value,
                val_auc_lr=val_auc_lr,
                test_auc=test_auc_value,
                ...
                n_train=[n_train_0, n_train_1],
                n_val=[n_val_0, n_val_1],
                n_test=[n_test_0, n_test_1],
                train_val_test_frac=[
                    config.train_frac,
                    config.val_frac,
                ...
                normalization_mean_dict=norm_mean_dict,
                normalization_std_dict=norm_std_dict,
                ct_b_min_max=[config.ct_b_min, config.ct_b_max],
                rtdose_b_min_max=[config.rtdose_b_min, config.rtdose_b_max],
                seg_orig_labels=seg_orig_labels,
                seg_target_labels=seg_target_labels,
                interpolation_mode_3d=[
                    config.ct_interpol_mode_3d,
                    config.rtdose_interpol_mode_3d,
                    config.segmentation_interpol_mode_3d,
                ],
                interpolation_mode_2d=[
                    config.ct_interpol_mode_2d,
        ...
        logger.my_print(
            "Elapsed time: {time} seconds".format(
                time=round(end - start, nr_of_decimals)
            )
        )
        ...
        if (fold == cv_folds - 1) and (cv_folds > 1):
            ...
            if max_epochs > 0:
                ...
                train_auc_list_mean = round(
                    sum(train_auc_list) / len(train_auc_list), nr_of_decimals
                )

            # LR training (avg)
            train_auc_lr_list = [x for x in train_auc_lr_list if x is not None]
            train_auc_lr_list_mean = round(
                sum(train_auc_lr_list) / len(train_auc_lr_list), nr_of_decimals
            )
            ...
            if max_epochs > 0:
                ...
                val_auc_list_mean = round(
                    sum(val_auc_list) / len(val_auc_list), nr_of_decimals
                )

                # Test (ensemble)
                test_auc_mean = misc.compute_auc(
                    y_pred_list=test_y_pred_ens,
                    y_true_list=test_y_ens,
                    auc_metric=auc_metric,
                )
            ...
            val_auc_lr_list_mean = round(
                sum(val_auc_lr_list) / len(val_auc_lr_list), nr_of_decimals
            )
            # dst_folder_name += '_lr_{}'.format(val_auc_lr_list_mean)
            # Test (ensemble)
            test_auc_lr_mean = misc.compute_auc(
                y_pred_list=test_y_pred_lr_ens,
                y_true_list=test_y_lr_ens,
                auc_metric=auc_metric,
            )
